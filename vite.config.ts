...
/**
 * Meta Pixel: só entra no HTML quando VITE_META_PIXEL_ID está definido.
 *
 * O id do Pixel é público (fica visível no HTML). O token do Conversions API
 * é outra coisa: vive apenas no servidor, em META_CAPI_ACCESS_TOKEN.
 */
function vitePluginMetaPixel(pixelId: string): Plugin {
  return {
    name: "scalio-meta-pixel",
    transformIndexHtml() {
      if (!pixelId) return [];
      return [
        {
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [
      ...
      vitePluginMetaPixel(env.VITE_META_PIXEL_ID || ""),
      ...devOnlyPlugins,
    ],
    resolve: {
      alias: {
